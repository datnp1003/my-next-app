...
import { getProductByCategory1 } from '@/core/domain/product/api';
import { useQuery } from '@tanstack/react-query';

export default function ProductCarousel() {
  const [current, setCurrent] = useState(0);

  const { data: products1 } = useQuery({
    queryKey: ['products1'],
    queryFn: () => getProductByCategory1(),
  });

  const productList1 = Array.isArray(products1) ? products1 : [];

  const nextSlide = () => {
    setCurrent(current === productList1.length - 1 ? 0 : current + 1);
  };

  const prevSlide = () => {
    setCurrent(current === 0 ? productList1.length - 1 : current - 1);
  };

  useEffect(() => {
    if (productList1.length === 0) return;
    const timer = setInterval(nextSlide, 5000);
    return () => clearInterval(timer);
  }, [current, productList1.length]);
    ...
    <div className="py-16 bg-white" id="products">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mt-20 grid grid-cols-1 gap-8 sm:grid-cols-2 lg:grid-cols-3">
            ...
            <div className="relative w-full h-96 rounded-lg overflow-hidden">
              {productList1.length > 0 && (
                <div className="absolute inset-0 flex transition-transform duration-500 ease-in-out"
                  ...
                  {productList1.map((item: any, index: number) => (
          ...
          {productList1.slice(0, 1).map((item: any, idx: number) => (
          ...
          {productList1.slice(1, 2).map((item: any, idx: number) => (
            <div key={idx} className="relative p-6 bg-white border border-gray-200 rounded-lg shadow-sm flex flex-col items-center">
              <div className="aspect-w-4 aspect-h-5 w-full overflow-hidden rounded-lg relative h-96">
                {item.images?.split(',').filter((x: any) => x).slice(0, 1).map((imageUrl: string, imageIndex: number) => (
                  <Image
